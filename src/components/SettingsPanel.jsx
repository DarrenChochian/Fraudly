...
export default function SettingsPanel({ currentHotkey: propHotkey = 'Alt+K', onHotkeyChange, onClose, onInteractiveEnter, onInteractiveLeave }) {
  ...
  // Fetch current hotkey when panel opens so it displays the actual keybind
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = async (e) => {
      ...
      if (window.electronAPI?.updateHotkey) {
        ...
        // Browser/dev mode — just update display
        setCurrentHotkey(accelerator)
        onHotkeyChange?.(accelerator)
        setStatus('saved')
      }
