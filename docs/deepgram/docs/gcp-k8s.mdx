...
Next, we need to create a Google Cloud Persistent Disk to hold the Deepgram model files. We'll attach this disk to a temporary Google Cloud VM, SSH into it, create a filesystem on the disk, download the model file(s), and then terminate the VM. The Persistent Disk will still remain, and will later be mounted to the GKE cluster when Deepgram containers are deployed.
