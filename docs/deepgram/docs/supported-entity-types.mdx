...
Individual entity classes can be redacted by specifying `redact=entity_class` one or more times.

<Info>
  Previously, redacting individual entity classes was only supported on the Deepgram hosted service. It is now also supported for self-hosted customers.
</Info>
