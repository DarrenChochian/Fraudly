...
| `agent.think.provider.reasoning_mode` | String            | Optional. Controls the reasoning effort for supported OpenAI reasoning models. Accepts `low`, `medium`, or `high`.                                                                                                               |
...
* Choose your language parameters based on your use case:
  ...
  * If you expect multiple languages or are unsure, use `multi` in `agent.listen.provider.language` for flexible language support.
  ...
  "agent": {
    ...
    "speak": {
      "provider": {
        ...
        "language": "en", // For Cartesia or Eleven Labs provider only, not needed for Deepgram Aura-2
