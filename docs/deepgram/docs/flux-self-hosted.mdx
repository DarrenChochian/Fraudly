...
subtitle: >-
Flux is a purpose-built, low-latency streaming speech-to-text model tailored
for voice agent use cases. This article describes how to ensure Flux is
present in your self-hosted Deepgram environment, the configuration steps, and
key considerations unique to Flux.
slug: docs/flux-self-hosted
---------------------------
...
Flux can automatically derive a maximum number of concurrent streams based on available GPU memory. However, for optimal and predictable performance, you must explicitly configure the `flux.max_streams` value in your Engine configuration file.

The appropriate value depends on your GPU type and workload characteristics and should be set according to Deepgram's benchmarking recommendations. Explicitly configuring this value ensures that concurrency limits reflect both memory and compute constraints, resulting in stable throughput and consistent latency under load.
...
For Deepgram self-hosted setups, there is a single model file that you'll need for Flux.
You can request this from your Deepgram account representative or Deepgram Support.
